...
def load_settings(path=SETTINGS_PATH):
    try:
        ...
        syringe_remaining_ul     = float(data.get("syringe_remaining_ul", syringe_volume_ul))
        syringe_full_travel_time_s = float(data.get("syringe_full_travel_time_s", 0.0))
        syringe_step_time_s      = float(data.get("syringe_step_time_s", 0.0))
        if syringe_step_time_s <= 0 and syringe_volume_ul > 0 and syringe_step_ul > 0 and syringe_full_travel_time_s > 0:
            syringe_step_time_s = syringe_full_travel_time_s * (syringe_step_ul / syringe_volume_ul)
...
class StepRecorder:
    def __init__(self, root_dir, meta, step_index,
                 record_telemetry=True, record_video=True):
    ...
    def _setup(self):
        ...
        if self.record_video:
            ...
            self.video_writers = {"rs": None, "uvc1": None, "uvc2": None}
            self.frame_counts = {"rs": 0, "uvc1": 0, "uvc2": 0}

        self.active = True

    def log_telemetry(self, t_rel, joints, joystick, extra):
        if not self.active or not self.telemetry_file:
            return
        (lx, ly, rx, ry, hx, hy,
    ...
    def _ensure_writer(self, cam_key, frame):
        if not self.record_video or self.video_writers.get(cam_key):
            return
        h, w = frame.shape[:2]
    ...
    def write_frame(self, t_rel, cam_key, frame):
        if not self.active or not self.record_video or frame is None:
            return
...
def draw_injection_info_modal(screen, study_text, material_text, date_text,
                              active_field, record_telemetry, record_video):
    """
    Start Injection popup: only Study, Injection Material, Date
    plus checkboxes for telemetry & video.
    """
    sw, sh = screen.get_size()
    font = pygame.font.SysFont("Consolas", 16)
    ...
    def field(ypos, text, label, key):
        ...
        box = pygame.Rect(x + 16 + lbl.get_width() + 8, ypos - 4,
                          panel_w - lbl.get_width() - 40, 30)
    ...
    if record_telemetry:
        pygame.draw.line(screen, (0,255,127), (cb1.x+3, cb1.y+cb_size//2),
                         (cb1.x+cb_size//2, cb1.y+cb_size-3), 2)
        pygame.draw.line(screen, (0,255,127), (cb1.x+cb_size//2, cb1.y+cb_size-3),
    ...
    if record_video:
        pygame.draw.line(screen, (0,255,127), (cb2.x+3, cb2.y+cb_size//2),
                         (cb2.x+cb_size//2, cb2.y+cb_size-3), 2)
        pygame.draw.line(screen, (0,255,127), (cb2.x+cb_size//2, cb2.y+cb_size-3),
                         (cb2.x+cb_size-3, cb2.y+3), 2)
    lbl2_lines = wrap_text("Record video (RealSense + both UVC feeds)", font, panel_w - cb2.right - 16)
    ty = cb2.y + (cb_size - len(lbl2_lines)*font.get_height())//2
    for ln in lbl2_lines:
        t = font.render(ln, True, label_color)
        screen.blit(t, (cb2.right + 8, ty))
        ty += font.get_height()
    ...
    return (
        ...
        {"telemetry": cb1, "video": cb2},
        panel
    )

def draw_subject_modal(screen, subject_text):
    ...
    title = font.render("Subject / Animal for this injection step", True, (230,230,230))
    screen.blit(title, (x + 16, y + 14))

    info_lines = [
        "Each syringe step can have a different Subject/Animal ID.",
    ...
    box = pygame.Rect(x + 16, ty + 30, panel_w - 32, 34)
    pygame.draw.rect(screen, (50,50,80), box)
    pygame.draw.rect(screen, (0,255,127), box, 2)
    ...
    btn_h = 30
    bx = x + panel_w - (btn_w*2 + 16 + 10)
    by = y + panel_h - btn_h - 14
    btn_confirm = pygame.Rect(bx, by, btn_w, btn_h)
    btn_cancel  = pygame.Rect(bx + btn_w + 10, by, btn_w, btn_h)

    for rect, text in [(btn_confirm, "Confirm"), (btn_cancel, "Cancel")]:
        pygame.draw.rect(screen, (50,50,80), rect)
        pygame.draw.rect(screen, (0,255,127), rect, 2)
        tt = font.render(text, True, (230,230,230))
        screen.blit(tt, (rect.x + (rect.w - tt.get_width())//2,
    ...
    return box, {"confirm": btn_confirm, "cancel": btn_cancel}, panel
...
def main():
    ...
    pygame.display.set_caption("Robotic Injector — RS+UVC (routine + syringe calibration + logging)")
    ...
    with state.lock:
        ...
        s_tot   = state.settings.get("syringe_rotations_total", 60.0)
        s_step  = state.settings.get("syringe_step_ul", 1.0)
        s_step_rot = state.settings.get("syringe_step_rotations", 6.0)
        s_remaining = state.settings.get("syringe_remaining_ul", s_vol)
        inj_active  = state.settings.get("injection_active", False)
    ...
    injection_step_deadline    = 0.0  # legacy
    ...
    def start_injection_session():
        ...
        nonlocal injection_record_telemetry, injection_record_video
        ...
        with state.lock:
            ...
            prev_subject = s.get("injection_subject", "")
        ...
        if injection_active_now:
            # Immediately ask Subject/Animal once after starting injection
            subject_field_text = injection_subject_text or prev_subject or ""
    ...
    while running:
            ...
            if ev.type == pygame.KEYDOWN:
                ...
                if syringe_calib_active:
                    if ev.key == pygame.K_ESCAPE:
                        ...
                        syringe_field_active = "step" if syringe_field_active == "vol" else "vol"
                    elif ev.key == pygame.K_RETURN:
                ...
                if injection_info_active:
                    if ev.key == pygame.K_ESCAPE:
                        ...
                        injection_field_active = cycle.get(injection_field_active, "study")
                    elif ev.key == pygame.K_RETURN:
        ...
        # Distance-based integration for an active injection step
        if injection_active_flag and injection_step_in_progress and ser and not calibration_running_flag:
            ...
            if injection_step_target > 0 and injection_step_distance >= injection_step_target:
                ...
                with state.lock:
                    ...
                    small = injection_last_step_small or (step_nom > 0 and injection_step_volume_ul < step_nom - 1e-6)
                ...
                if injection_last_step_small:
                    try:
                        ...
                        pass
                print(f"[Injection] Step complete: {injection_step_volume_ul:.3f} µL, remaining {new_remaining:.3f} µL")
        ...
        if a_edge and not post_step_prompt_active:
        ...
        if b_edge and not post_step_prompt_active:
        ...
        if ser:
            ...
            if plunger_bridge_allowed and not injection_active_flag and not calibration_running_flag:
        ...
        if injection_active_flag and ser and not calibration_running_flag:
            with state.lock:
                syringe_rot_total = float(state.settings.get("syringe_rotations_total", 0.0) or 0.0)
                vol_total = float(state.settings.get("syringe_volume_ul", 0.0) or 0.0)
            ...
            if syringe_rot_total <= 0 or vol_total <= 0:
                with state.lock:
                    ...
                    state.calibration_status = "Injection disabled: syringe stroke/volume not calibrated."
                save_settings(state.settings)
                injection_step_in_progress = False
                injection_step_done = False
                post_step_prompt_active = False
                ...
                # Start a new step only if no modal subject popup is active
                if (not injection_step_in_progress) and (not injection_step_done) and (not post_step_prompt_active) and (not subject_prompt_active):
                    if right_down and not last_right_trigger_down:
                        if syringe_remaining_ul >= syringe_step_ul + 1e-6:
                            step_vol = syringe_step_ul
                            is_small = False
                        else:
                            step_vol = syringe_remaining_ul
                            is_small = True
                        ...
                        injection_last_step_small = is_small
                        ...
                        with state.lock:
                            ...
                            inj_meta = {
                                "study": state.settings.get("injection_study",""),
                                "subject": state.settings.get("injection_subject",""),
                                "material": state.settings.get("injection_material",""),
                                "date": state.settings.get("injection_date",""),
                            }
                            do_tel = rec_tel
                            do_vid = rec_vid
                        if do_tel or do_vid:
                            try:
                                current_step_recorder = StepRecorder(
                                    ...
                                    record_telemetry=do_tel, record_video=do_vid
                                )
                            except Exception as e:
                                print(f"[Recorder] Failed to create recorder: {e}")
                                current_step_recorder = None

                        print(f"[Injection] Step {step_index} started: {step_vol:.3f} µL (target distance {injection_step_target:.1f})")

                if injection_step_in_progress and not post_step_prompt_active:
                    with state.lock:
                        plunger_fwd_rate = int(state.settings.get("plunger_fwd_rate", MAX_RATE) or MAX_RATE)
                        dir_sign = int(state.settings.get("plunger_dir_sign", 1) or 1)
                    desired_rate = 0
                    if right_down:
                        rt_val = normalize(cur_rt)
                        if rt_val > DEADZONE:
                            desired_rate = -int(abs(plunger_fwd_rate) * rt_val) * dir_sign
                ...
                if (not injection_step_in_progress or post_step_prompt_active) and last_rate not in (0, None):
                    try:
                        ser.write(b"V0\n")
                        if ser.in_waiting:
                            ser.read(ser.in_waiting)
        ...
        vf = pygame.font.SysFont("Consolas", 12)
        ...
        inj_label = f"Injection: {'ACTIVE' if injection_active_flag else 'idle'}; ammo={ammo_steps} steps"
        ...
        texts = [
            ...
            vf.render(inj_label, True, (255,220,180)),
            vf.render(f"Syringe calib: {calibration_status}", True, (200,230,200)),
        ]
        ...
        # Discrete ammo indicator (bottom-right, bar-style)
        if syringe_step_ul > 0 and syringe_volume_ul > 0:
            ...
            if frac_bar_disp > 1e-3:
                ...
                bx += bar_w_pix + gap_pix
            ammo_txt = vf.render(f"{ammo_float:.1f} step(s)", True, (230,230,230))
            ammo_surf.blit(ammo_txt, (bx + 4, by))
            vol_txt = vf.render(f"{syringe_remaining_ul:.2f}/{syringe_volume_ul:.2f} µL", True, (200,220,255))
            ammo_surf.blit(vol_txt, (10, by + bar_h_pix + 6))
            if remainder_ul_for_overlay > 1e-3:
                warn = vf.render(f"Final≈{remainder_ul_for_overlay:.2f} µL", True, (255,200,120))
                ammo_surf.blit(warn, (panel_w_ammo - warn.get_width() - 10, by + bar_h_pix + 6))
        ...
        for b in buttons:
            ...
            if kind == "pose_row":
                ...
                go_txt_lines = wrap_text(f"Go: {name}", placeholder_font, go_w - 10)
                ly2 = go_rect.y + 4
        ...
        if current_step_recorder and current_step_recorder.active:
            ...
            with state.lock:
                joints_snapshot = state.last_joints if isinstance(state.last_joints, dict) else (state.last_joints or {})
            ...
            joystick_tuple = (lx_s, ly_s, rx_s, ry_s,
                              hx_s, hy_s,
                              lt_val, rt_val_norm,
                              lb_s, rb_s,
        ...
        if current_step_recorder and (not injection_step_in_progress) and (not post_step_prompt_active) and (not injection_step_done):
        ...
        if click_pos is not None and not modals_open:
            ...
            if handled:
                ...
                _, tr_rect2, _, _ = compute_layout(*pygame.display.get_surface().get_size(), args.ui_frac, args.ui_min_width)
                    ...
                    for rect, act in ui_clickables:
                        if rect.collidepoint(mx, my):
                            ...
                            if kind == "edit_tool":
                                ...
                                if routine_executor and routine_executor.is_alive():
                                    routine_executor.stop()
                                rt.enqueue({"type":"stop_motors"})  # now HALT, not motor-off
                                if ser:
                                    try:
                                        ser.write(b"V0\n")
                                        if ser.in_waiting:
        ...
        if syringe_calib_active:
            ...
            field_rects, button_rects, panel_rect = draw_syringe_calibration_modal(
                screen, syringe_vol_text, syringe_step_ul_txt, status_text, syringe_field_active
            )
        ...
        if injection_info_active:
            field_rects2, button_rects2, checkbox_rects2, panel_rect2 = draw_injection_info_modal(
                screen,
        ...
        if subject_prompt_active:
            input_rect, btn_rects_subj, subj_panel = draw_subject_modal(screen, subject_field_text)
            if click_pos is not None:
                mx, my = click_pos
                if not subj_panel.collidepoint(mx, my):
                    subject_prompt_active = False
                else:
                    if btn_rects_subj["cancel"].collidepoint(mx, my):
                        subject_prompt_active = False
                    elif btn_rects_subj["confirm"].collidepoint(mx, my):
                        subj = subject_field_text.strip()
                        injection_subject_text = subj
                        with state.lock:
                            state.settings["injection_subject"] = subj
        ...
        if post_step_prompt_active:
            with state.lock:
                s = state.settings
                syringe_volume_ul_overlay   = s.get("syringe_volume_ul", 10.0)
                syringe_step_ul_overlay     = s.get("syringe_step_ul", 1.0)
                syringe_remaining_ul_overlay= s.get("syringe_remaining_ul", syringe_volume_ul_overlay)

            if syringe_step_ul_overlay > 0:
                steps_left = max(0, int(syringe_remaining_ul_overlay // syringe_step_ul_overlay))
                remainder_ul = max(0.0, syringe_remaining_ul_overlay - steps_left * syringe_step_ul_overlay)
            else:
                steps_left = 0
                remainder_ul = 0.0
            ...
            if syringe_step_ul_overlay > 0:
                lines.append(f"Remaining full steps: {steps_left} × {syringe_step_ul_overlay:.3f} µL")
            ...
            if injection_last_step_small:
                lines.append(f"WARNING: last step was SMALL ({injection_step_volume_ul:.3f} µL)")
            lines.append("Retract toolhead by 10 mm?")
            lines.append("A = retract, B = skip   (Y = stop recording)")

            color = (255,80,80) if injection_last_step_small else (255,255,255)
            y0 = screen_h//2 - (len(lines)*(big_font.get_height()+6))//2
            for ln in lines:
            ...
            if a_edge:
                with state.lock:
                    approach_mm_local = state.approach_mm
                try:
                    rt._tool_move_along_tz(-10.0, cont=0)
                    print("[Injection] Toolhead retracted by 10 mm.")
                except Exception as e:
                    print(f"⚠️ Retraction after injection step failed: {e}")
                post_step_prompt_active = False
                injection_step_done = False
            ...
            elif y_now and not prev_y:
                # Y button: explicitly stop recording for this step, if active
        ...
        if waiting:
            ...
            txt1_lines = wrap_text(confirm_msg or "Paused at Midway. A: advance  B: free control", big_font, screen_w - 40)
            screen.blit(overlay, (0,0))
            y0 = screen_h//2 - (len(txt1_lines)*(big_font.get_height()+4))//2
